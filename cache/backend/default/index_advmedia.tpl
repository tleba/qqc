     <div id="rightcontent">
        ...
        <div id="right">
            ...
            <tr bgcolor="{cycle values="#F8F8F8,#F2F2F2"}">
                ...
                <td align="center">{math equation="(( z * x ) / z )" x=$advs[i].views y=$advs[i].clicks z=100 format="%.2f"}%</td>
                <td align="center">{$advs[i].addtime|date_format}</td>
                <td align="center"><b>{if $advs[i].status == '1'}Active{else}Suspended{/if}</b></td>
                <td align="center">
					<a href="index.php?m=advmediaedit&AID={$advs[i].adv_id}">Edit</a><br>
                    <a href="index.php?m=advmedia&a=delete&AID={$advs[i].adv_id}" onClick="javascript:return confirm('Are you sure you want to delete this advertising media?');">Delete</a><br>
                    {if $advs[i].status == '1'}
                    <a href="index.php?m=advmedia&a=suspend&AID={$advs[i].adv_id}" onClick="javascript:return confirm('Are you sure you want to suspend this advertising media?');">Suspend</a>
                    {else}
                    <a href="index.php?m=advmedia&a=activate&AID={$advs[i].adv_id}" onClick="javascript:return confirm('Are you sure you want to activate this advertising media?');">Activate</a>
                    {/if}
                    <br>
                </td>
            </tr>
