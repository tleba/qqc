$(document).ready(function(){            
    $("[id*='vote_']").click(function(event) {
        ...
        $.post(base_url + '/ajax/vote_game', { item_id: item_id, vote: vote },
            function (response) {
			if (response.msg =='') {
			if (response.likes != 0 || response.dislikes != 0) {
				...
				$("#game_rate").css("width", response.rate + "%");
                if (response.likes != $("#game_likes").text()) {
					$("#game_likes").animate({'opacity': 0}, 200, function () {
					$(this).text(response.likes);
					}).animate({'opacity': 1}, 200);
				}
                if (response.dislikes != $("#game_dislikes").text()) {
					$("#game_dislikes").animate({'opacity': 0}, 200, function () {
