...
$.fn.jqPuzzle = function(settings, texts) {
	...
	var defaults = {
		...
		control: {
			...
			beforecountercallback:undefined,	//移动每步前的回调
			aftercountercallback:undefined,	//移动每步后的回调
			startcallback:undefined     //游戏开始或重新开始洗牌的回调
		},
	...
	var i18n = {
		...
		pt: {
			...
			toggleOriginalLabel:	'',
			toggleNumbersLabel:		'',
			confirmShuffleMessage:	'你真的想要洗牌吗?',
			movesLabel:				'步',
			secondsLabel:			'秒'
		}
	};
		...
		$srcImg.one('load', function() {
			...
			var shuffle = function(rounds, speed) {
				...
				if(speed) {
					...
					var result = control.startcallback({title:settings.title,isstart:true,isrestart:(moves > 0)});
					if(!result)
						return false;
					settings.gid = result.gid;
					lock = true; // set lock
			...
			var $protoButton = $('<a/>').css('cursor', 'pointer');
			...
			$pieces.click(function() {
				...
				if(!control.beforecountercallback({gid:settings.gid,title:settings.title,moves: moves, seconds: seconds}))
					return false;
				...
				$piece.animate({left: offset.left, top: offset.top}, animation.slidingSpeed, null, function() {
					control.aftercountercallback({gid:settings.gid,title:settings.title,left: offset.left, top: offset.top,moves: moves, seconds: seconds});
			...
			var solutionCallback = function() {
				if($.isFunction(success.callback)) {
					setTimeout(function() {
						success.callback({gid:settings.gid,title:settings.title,moves: moves, seconds: seconds});
