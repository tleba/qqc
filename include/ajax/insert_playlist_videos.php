    ...
    if ( isset($_SESSION['uid']) ) {
        ...
        if ( $videos ) {
            ...
            foreach ( $videos as $video ) {
                ...
                $code[] = '<a href="#attach_playlist_video_' .$video['VID']. '" id="attach_media_video_' .$video['VID']. '"><img src="' .$config['BASE_URL']. '/media/videos/tmb/' .$video['VID']. '/1.jpg" alt="' .htmlspecialchars($video['title'], ENT_QUOTES, 'UTF-8'). '" class="img-responsive" /></a>';
                $code[] = '</div>';
            }
            $code[]     = '</div>';
            $code[]     = '</div>';		
