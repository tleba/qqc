...
if ( isset($_POST['photo_id']) && isset($_POST['page']) ) {
	...
	$code[]         = '<div id="photo_response" class="response" style="display: none;"></div>';
    ...
    if ( $comments ) {
        ...
        foreach ( $comments as $comment ) {
            ...
            $code[]     = '<div id="photo_comment_' .$pid. '_' .$comment['CID']. '" class="col-xs-12 m-t-15">';
			$code[]		= '<div class="row">';
            ...
            if ( $uid ) {
                ...
                if ( $comment['UID'] == $uid ) {
                    $code[] = '<a href="#delete_comment" id="delete_comment_photo_' .$comment['CID']. '_' .$pid. '">'.$lang['global.delete'].'</a> <span id="delete_response_' .$comment['CID']. '" style="display: none;"></span>';
                }
				if ( $comment['UID'] != $uid ) {
					$code[]  = '<span id="reported_spam_' .$comment['CID']. '_' .$pid. '"><a href="#report_spam" id="report_spam_photo_' .$comment['CID']. '_' .$pid. '">'.$lang['global.report_spam'].'</a></span>';
				}
                $code[]  = '</div>';
            }
			$code[]     = '</div>';			
