    ...
    if ( isset($_SESSION['uid']) ) {
        ...
        if ( $conn->Affected_Rows() === 1 ) {
            ...
            if ( $album['UID'] == $uid ) {
                ...
                $conn->execute($sql);
                $response['status'] = 1;
				$response['msg'] = show_msg_mb($lang['ajax.delete_photo_success']);
            } else {
                $response['msg'] = show_err_mb($lang['ajax.delete_photo_failed']);
            }
        } else {
            $response['msg'] = show_err_mb($lang['ajax.delete_photo_failed']);
        }
        ...
        $response['msg']   = show_err_mb($lang['ajax.delete_photo_login']);
    }
}
