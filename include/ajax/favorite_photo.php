...
if ( isset($_POST['photo_id']) && isset($_POST['album_id']) ) {
    ...
    if ( isset($_SESSION['uid']) ) {
        ...
        if ( $conn->Affected_Rows() == 1 ) {
            $data['msg']    = show_err($lang['ajax.favorite_photo_exists']);
        } else {
            $sql        = "SELECT PID FROM photos WHERE PID = " .$photo_id. " AND UID = " .$uid. " LIMIT 1";
            $conn->execute($sql);
            if ( $conn->Affected_Rows() == 1 ) {
                $data['msg']    = show_err($lang['ajax.favorite_photo_self']);
            } else {
                ...
                $conn->execute($sql);
                $data['msg']    = show_msg($lang['ajax.favorite_photo_success']);
                $data['status'] = 1;
            }
        }
    } else {
        $data['msg']        = show_err($lang['ajax.favorite_photo_login']);
    }
}
