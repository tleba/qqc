...
*/
	...
	class ADODB_Cache_MemCache {
		...
		var $hosts = '127.0.0.1';	// array of hosts
		var $port = 11211;
		...
		function writecache($filename, $contents,$debug, $secs2cache,$expire=14400)
		{
			if (!$this->_connected) {
				$err = '';
			...
			if (!$this->_memcache->set($filename, $contents, $this->compress, $expire)) {
		...
		function flushcache($filename, $debug=false)
		{
			...
			if ($debug) 
				if (!$del) ADOConnection::outp("flushcache: $filename entry doesn't exist on memcached server!<br>\n");
				else ADOConnection::outp("flushcache: $filename entry flushed from memcached server!<br>\n");
