...
$categories     = get_lists_categories(0);
...
switch ( $order ) {
    case 'bw':
        $title_o  = ' '.$lang['global.mores'];
        $sql_add .= ' ORDER BY total_imgs DESC';
        break;
    case 'mr':
...
//取数据
$sql            = "SELECT count(VID) AS total_videos FROM picture " .$sql_add_count;

$rsc            = $conn->CacheExecute(3000,$sql);
$total          = $rsc->fields['total_videos'];
$pagination     = new Pagination($config['videos_per_page']);
$limit          = $pagination->getLimit($total);
$sql            = "SELECT * FROM picture " . $sql_add . " LIMIT " . $limit;

$rs             = $conn->CacheExecute(3000,$sql);
$pictures       = $rs->getrows();
$page_link      = $pagination->getPagination('pictures');
...
if ($isMakeHtml == 1) {
	...
	$content .= $smarty->fetch('pictures.tpl', null, null, false);
